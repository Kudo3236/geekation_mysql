...
SELECT region,AVG(life_expectancy),AVG(population) FROM countries
GROUP BY region;

-- 問22
-- 全ての地方の最長寿命、最大人口を表示してください。(NULLも表示)
SELECT region,MAX(life_expectancy),MAX(population) FROM countries
GROUP BY region;


-- 問23
-- アジア大陸の中で最小の表面積を表示してください
SELECT MIN(surface_area) FROM countries
...
SELECT countries.name,countrylanguages.language FROM countries
...
AS language_max 
ON countrylanguages.country_code = language_max.country_code
AND max_percentage = language_max.max_percentage;
